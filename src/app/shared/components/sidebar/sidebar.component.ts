 ...
 * sezione auth in fondo (Bonus B): username/role + logout per l'utente
 * loggato, oppure link "Accedi" se nessuno e' autenticato.
 *
 * Comportamento collassabile:
      ...
      @if (isOpen()) {
        <div class="border-t border-slate-800 p-3">
          @if (auth.currentUser(); as user) {
            ...
            <a
              routerLink="/login"
              class="block w-full rounded-md bg-red-500 px-3 py-1.5 text-center text-xs font-medium text-white hover:bg-red-600"
            >
              Accedi
            </a>
          }
